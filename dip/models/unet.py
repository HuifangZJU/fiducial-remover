...
class UNet_Parrel(nn.Module):
    '''
        upsample_mode in ['deconv', 'nearest', 'bilinear']
        pad in ['zero', 'replication', 'none']
    '''
    def __init__(self, pack_size,num_input_channels=3, num_output_channels=3,
                       feature_scale=4, more_layers=0, concat_x=False,
                       upsample_mode='deconv', pad='zero', norm_layer=nn.InstanceNorm2d, need_sigmoid=True, need_bias=True):
        super(UNet_Parrel, self).__init__()
        self.image_channel = num_input_channels
        self.pack_size = pack_size
        self.nets=[]

        self.nets = nn.ModuleList([UNet(num_input_channels, num_output_channels,
                           feature_scale, more_layers, concat_x,
                           upsample_mode, pad, norm_layer, need_sigmoid, need_bias) for i in range(self.pack_size)])

    def forward(self, inputs):
        image = inputs[:, :self.image_channel, :, :]
        net = self.nets[0]
        outputs = net(image)
        for i in range(1,self.pack_size):
            image = inputs[:,i*self.image_channel:(i+1)*self.image_channel,:,:]
            net = self.nets[i]
            output = net(image)
