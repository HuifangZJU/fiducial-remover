...
def detect_contour(id,img,counts,img_file,scale):
    ...
    if id==1:
        ...
        # if os.path.exists(image_file_name[:-4] + '_cleaned_with_bg.png'):
        #     img_vispro = cv2.imread(img_file[:-4] + '_cleaned_with_bg.png')
        # else:
        #     img_vispro = cv2.imread(img_file[:-4] + '_cleaned_with_bg.tif')
        # img_vispro = cv2.imread(img_file[:-4] + '_0.png')
        img_vispro = readin_tiff(img_file[:-4] + '_0.tif',scale)
...
def run_single_sample(counts_file,img_file):
    ...
    img = readin_tiff(img_file,scale)
    ...
    for i in [4]:
        print(i)
        img,cnt = detect_contour(i,img,counts,img_file,scale)
        ...
        enhanced_exp_adata.write_h5ad(counts_file[:-5] + "_res"+str(res)+"_imputed_"+str(i)+".h5ad")
...
for i in range(1,num_files):
    ...
    if not os.path.exists(image_file_name[:-4] + '_cleaned_with_bg.png') and not os.path.exists(image_file_name[:-4] + '_cleaned_with_bg.tif'):
        continue

    adata_file_name = "/media/huifang/data/fiducial/tiff_data/Visium_Human_Breast_Cancer_spatial/Visium_Human_Breast_Cancer_raw_feature_bc_matrix.h5ad"
    image_file_name = "/media/huifang/data/fiducial/tiff_data/Visium_Human_Breast_Cancer_spatial/Visium_Human_Breast_Cancer_image.tif"
